...
# Conventional Commit subjects merged to main since then (this repo squash-
# or rebase-merges, so a PR title is (or becomes) the commit subject).
#
...
# Prints the next version (e.g. "0.3.0") to stdout and exits 0.
...
# also match a prerelease tag like "v0.2.1-rc1" (the trailing [0-9]* happily
...
if [ -z "$last_tag" ]; then
  echo "::error::No vX.Y.Z tag found to compute the next version from -- push an initial v<current-workspace-version> tag once to bootstrap" >&2
  exit 2
fi
...
# silently downgrading what should be a major bump. Verified: the piped
# form actually misses the match on a >1MB body; the here-string doesn't.
